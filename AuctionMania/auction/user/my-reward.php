...
$today= date("Y-m-d");
$sql8="select * from assemble a, product p where p.p_end <= $today and p.pid = a.pid ";
...
//echo $uid;
      $today= date("Y-m-d");
$sql8="select * from assemble a, product p, category c, user u where u.uid = a.uid and a.pid = p.pid and c.cid = a.cid and temp <= '$today' and a.uid in (select uid from assemble where uid = '$uid' and h_price in (select MAX(h_price) from assemble where uid = '$uid') group by uid) ";
$sql9="select pid,MAX(h_price) as h_price from assemble group by pid";
...
while($row8=mysqli_fetch_array($res8))
{
  ...
  {
    if($row9['pid']==$row8['pid'])
    {
        if($row9['h_price']==$row8['h_price'])
        {
  ...
  ?>
   ...
   <div class="box box-primary">
            <div class="box-header with-border">
              ...
              <h3 class="box-title">Product Name: &nbsp;<?php echo $row8['p_name'];?></h3>
            </div>
            ...
            <div class="box-body">
              ...
              <p><marquee>Congrats you have won this product and its your now.... for <?php echo $row8['h_price'];?></marquee></p></strong>
            </div>
